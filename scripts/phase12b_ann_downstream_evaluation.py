...
POOLS_FILE = ROOT / "results/phase12a_ann_candidate_pools.csv"
...
OUT_METRICS = ROOT / "results/phase12b_ann_downstream_metrics.csv"
OUT_QUERY = ROOT / "results/phase12b_ann_query_predictions.csv"
OUT_COMPONENT = ROOT / "results/phase12b_ann_component_predictions.csv"
OUT_SUMMARY = ROOT / "results/phase12b_ann_downstream_summary.json"
...
print("==========================================")
print("Phase 12B - ANN Downstream Evaluation")
print("==========================================")
...
# Exact pool is repeated on each Phase12A config row.
for qid, qgroup in pools.groupby("query_id"):

    exact_values = {
        clean(value)
        for value
        in qgroup["exact_candidate_pool"]
    }

    if len(exact_values) != 1:
        raise RuntimeError(
            f"{qid}: inconsistent exact candidate pool"
        )

    pools_by_backend["EXACT"][qid] = json.loads(
        next(iter(exact_values))
    )
...
for config in [
    ...
    cg = pools[
        pools["config"].astype(str)
        == config
    ]

    if len(cg) != 360:
        raise RuntimeError(
            f"{config}: expected 360 pool rows"
        )

    for row in cg.itertuples(index=False):
        pools_by_backend[config][
            clean(row.query_id)
        ] = json.loads(
...
summary = {
    "phase12b_complete": True,
    "scope":
        "ANN_RETRIEVAL_DOWNSTREAM_CONTENT_ONLY_EVALUATION",
    ...
    "interpretation_rule": {
        ...
        "balanced_not_promoted_based_on_test":
            "BALANCED, FAST, and HIGH_RECALL are reported as fixed operating points; TEST results are not used to select a final ANN configuration.",
    },
}
