...
# group requested classes per historical JAR
...
for gt in ground_truth_rows:
    ...
    for node_id, hidden in (
        ...
        # CONNECTOR content must NOT
        # become attribution evidence.
        if (
            hidden[
                "evaluation_role"
            ]
...
def package_parent_restricted(
    ...
    # Graph ignored.
    #
    # Rank-based package evidence only.
    # -----------------------------------------------------
...
def dependency_assignment(
    ...
    # -----------------------------------------------------
    # CONNECTOR nodes carry NO content score.
    #
    # They are used only to connect TARGET nodes.
    # -----------------------------------------------------
    ...
    for component in components:
        ...
        # -----------------------------------------------
        # Sum normalized regret.
        #
        # For each target:
        # cost(mod) - best_cost(node)
        #
        # This avoids tuning a temperature.
        # -----------------------------------------------
...
raw.to_csv(
    ...
    RESULT_ROOT
    / "multiparent_evaluation_raw.csv",
...
summary_df.to_csv(
    ...
    RESULT_ROOT
    / "multiparent_evaluation_summary.csv",
...
print(
    "Raw     : "
    "results\\multiparent_evaluation_raw.csv"
)

print(
    "Summary : "
    "results\\multiparent_evaluation_summary.csv"
)
