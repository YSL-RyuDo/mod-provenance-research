...
def main():
    ...
    for r in qmap:
        node_id = r["node_id"]
        stone_meta = stone_by_node[node_id]

        # Evaluation ground truth must use the frozen benchmark label.
        # Held-out sources still have an internal source_fresh_id, but their
        # evaluation label is UNKNOWN and must not be exposed to the baseline.
        gt_fresh = stone_meta["ground_truth_label"]

        gt_parent = (
            fresh_to_parent.get(gt_fresh, "")
            if gt_fresh != "UNKNOWN"
            else ""
        )
        ...
        gt_known = gt_fresh != "UNKNOWN"

        gallery_available = (
            gt_parent in parent_source_available
            if gt_known and gt_parent
            else False
        )
    ...
    for aqid, rows in sorted(by_q.items()):
        ...
        # Primary frozen benchmark contains at most one held-out
        # UNKNOWN provenance source per query. Therefore K is the number of
        # distinct registered parents plus one UNKNOWN group when present.
        k_pred = len(pred_known) + (1 if pred_has_unknown else 0)
        ...
        query_rows.append({
            ...
            "gt_has_unknown": gt_has_unknown,
            "pred_has_unknown": pred_has_unknown,
        })
